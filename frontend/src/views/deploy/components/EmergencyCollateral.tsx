...
import { Box, CardProps, Divider, Flex, IconButton, Select, Text } from "theme-ui";
...
const EmergencyCollateral = ({
    ...
    const addresses = useMemo(() => collaterals.map((c) => c.address), [collaterals]);

    const handleDiversityFactor = (e: any) => {
        updateBasket([targetUnit, { diversityFactor: +e.target.value, collaterals }]);
    };
    ...
    const handleRemove = (index: number) => {
        updateBasket([
            ...
            {
                diversityFactor:
                    diversityFactor === collaterals.length ? diversityFactor - 1 : diversityFactor,
                collaterals: [...collaterals.slice(0, index), ...collaterals.slice(index + 1)],
            },
        ]);
    };
    ...
    return (
        <Box {...props}>
            ...
            {readOnly ? (
                ...
                >
                    <SortableContext items={addresses} strategy={verticalListSortingStrategy}>
                        {collaterals.map((collateral, index) => (
