...
import { BackupBasket, backupCollateralAtom, Basket, basketAtom, deployIdAtom } from "../atoms";
    ...
    try {
        ...
        const config: RTokenConfiguration = {
            ...
            params: {
                ...
                backingBuffer: parseEther((Number(tokenConfig.backingBuffer) / 100).toString()),
                maxTradeSlippage: parseEther(
                    (Number(tokenConfig.maxTradeSlippage) / 100).toString()
                ),
                issuanceRate: parseEther((Number(tokenConfig.issuanceRate) / 100).toString()),
        ...
        for (const targetUnit of Object.keys(basket)) {
            ...
            collaterals.forEach((collateral, index) => {
                ...
                weights.push(
                    parseEther(((Number(distribution[index]) / 100) * Number(scale)).toString())
                );
            });

            if (backup[targetUnit] && backup[targetUnit].collaterals.length) {
                backups.push({
                    backupUnit: ethers.utils.formatBytes32String(targetUnit.toUpperCase()),
                    diversityFactor: parseEther(backup[targetUnit].diversityFactor.toString()),
...
const ConfirmDeploy = () => {
    ...
    const [fee, gasError, gasLimit] = useTransactionCost(transaction ? [transaction] : []);
