...
const Mining = ({ hash }: { hash: string }) => (
    <Box sx={{ textAlign: "center", width: 400 }}>
        ...
        <Text as="p" variant="legend">
            <Trans>
                Meditate peacefully on the stability of a future asset backed reserve currency
                while your ETF deploys 🧘‍♂️
            </Trans>
            <br />
            <br />
            <Trans>
        ...
        <Box variant="layout.verticalAlign" sx={{ justifyContent: "center" }} mt={4}>
...
const DeployStatus = () => {
    ...
    useEffect(() => {
        if (tx?.extra?.rTokenAddress && rToken?.address === tx.extra.rTokenAddress) {
