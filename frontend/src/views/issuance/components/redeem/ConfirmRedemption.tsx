...
const ConfirmRedemption = ({ onClose }: { onClose: () => void }) => {
    ...
    const [collateralQuantities, setCollateralQuantities] = useAtom(redeemCollateralAtom);
    ...
    const getQuantities = useCallback(async (facade: Facade, rToken: string, value: string) => {
        try {
    ...
    useEffect(() => {
        if (facadeContract && rToken?.address && !rToken.isRSV && Number(amount) > 0) {
