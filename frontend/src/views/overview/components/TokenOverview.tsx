    ...
    return (
        <Box {...props}>
            ...
            <Flex mt={6} sx={{ flexWrap: "wrap" }}>
                <Box mr={5}>
                    <InfoHeading mb={3} title={t`Market cap`} subtitle={metrics.supplyUsd} />
                    {!rToken?.isRSV && (
                        <InfoHeading title={t`Insurance Pool`} subtitle={metrics.insuranceUsd} />
                    )}
                </Box>
                {!rToken?.isRSV && (
                    <Box>
                        <InfoHeading mb={3} title={t`ETF Yield`} subtitle={`${tokenApy}%`} />
