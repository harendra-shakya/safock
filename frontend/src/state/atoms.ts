...
export const accountRTokensAtom = atom<{ address: string; name: string; symbol: string }[]>([]);
...
export const txAtom = atomWithStorage<TransactionMap>("transactions", {}, txStorage);
...
export const addTransactionAtom = atom(null, (get, set, tx: TransactionState[]) => {
    const txs = get(txAtom);
    ...
    const currentTx = txs[chainId] && txs[chainId][account] ? txs[chainId][account] : [];
