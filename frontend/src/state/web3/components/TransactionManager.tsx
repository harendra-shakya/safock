...
const getDeployedRToken = (receipt: TransactionReceipt): string => {
    const log = receipt.logs.find((logs) => logs.address === DEPLOYER_ADDRESS[CHAIN_ID]);
...
const getGovernance = (receipt: TransactionReceipt): StringMap => {
    const log = receipt.logs.find((logs) => logs.address === FACADE_WRITE_ADDRESS[CHAIN_ID]);

    if (log) {
        const { rToken, governance, timelock } = FacadeWriteInterface.parseLog(log).args;
