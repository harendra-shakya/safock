...
> = (props: any) => {
    ...
    return (
        <Box sx={{ display: "flex", alignItems: "center" }} variant="table.pagination.container">
            <Box sx={{ fontSize: 1, color: "lightText" }} ml={3}>
                {totalCountVisible && totalCountResolvedTemplate}
            </Box>
            <Box sx={{ marginLeft: "auto" }} variant="table.pagination.navigationContainer">
                ...
                {pageVisible && <Box variant="table.pagination.page">{pageResolvedTemplate}</Box>}
                {goToPageVisible && (
