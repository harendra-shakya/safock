...
const NavItem = ({ path, title, Icon, rTokenAddress, ...props }: NavItemProps) => (
    <NavLink
        style={({ isActive }) => ({
            paddingLeft: "5px",
            textDecoration: "none",
...
// Sidebar Navigation
const Navigation = ({ currentToken }: { currentToken?: ReserveToken | null }) => {
    ...
    return (
        <Box mt={5}>
            {pages.map((item: any) => (
                <NavItem key={item.path} {...item} rTokenAddress={currentToken?.address ?? ""} />
            ))}
